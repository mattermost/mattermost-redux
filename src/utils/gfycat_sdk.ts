...
const GfycatConstructor = require('gfycat-sdk');
...
export default function(key: string, secret: string): any {
    ...
    if (!key || !secret) {
        instance = new GfycatConstructor({client_id: defaultKey, client_secret: defaultSecret});
        return instance;
    }

    activeKey = key;
    activeSecret = secret;
    instance = new GfycatConstructor({client_id: key, client_secret: secret});
    return instance;
}
