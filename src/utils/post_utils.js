...
export function consolidateUserActivitySystemMessages(posts = {}, order = []) {
    ...
    let testPosts = order.reduce((acc, postId, index, arr) => {
        ...
        let {newPosts, newOrder} = acc;
        let newPost = {};
        if (isPostUserActivity(post.type)) {
            let postProps = post.props;
            postProps = {...postProps, type: post.type};
            console.log("postProps:", postProps)
            if (consolidatedPosts) {
                console.log("if (consolidatedPosts):", consolidatedPosts)
                let {message, system_ids, props} = consolidatedPosts;
                // let newProps = {...props};
                message += ` ${post.message}`;
                system_ids = [...system_ids, post.id];
                console.log("if (consolidatedPosts) props:", props)
                let {user_activities} = props;
                console.log("if (consolidatedPosts) user_activities:", user_activities)
                user_activities = [...user_activities, postProps];
                // userActivitiesProps.push(postProps);
                let newProps = {...props, user_activities};
                ...
                if (post.props.user_activities) {
                    console.log("if (post.props.user_activities):", post.props.user_activities)
                    // consolidatedPosts = post;
                    newPost[post.id] = post;
                    newOrder.push(post.id)
                } else {
                    props[POST_PROPS_USER_ACTIVITIES] = [postProps];
                    consolidatedPosts = {
                        ...post, 
                        system_ids: [post.id],
                        props
            ...
            if (consolidatedPosts) {
                newPost[consolidatedPosts.id] = consolidatedPosts;
                newOrder.push(consolidatedPosts.id)
                consolidatedPosts = null;
            }
        ...
        if (consolidatedPosts && (arr.length === index + 1 || consolidatedPosts.system_ids.length === POST_PROPS_USER_ACTIVITIES_MAX)) {
            if (consolidatedPosts.system_ids.length === POST_PROPS_USER_ACTIVITIES_MAX) {
                console.log("HIT 30")
            }
