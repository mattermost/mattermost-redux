...
export function canDeletePost(config, license, userId, post, isAdmin, isSystemAdmin) {
    ...
    if (license.IsLicensed === 'true') {
        return (config.RestrictPostDelete === Constants.PERMISSIONS_DELETE_POST_ALL && (isOwner || isAdmin)) ||
            (config.RestrictPostDelete === Constants.PERMISSIONS_DELETE_POST_TEAM_ADMIN && isAdmin) ||
