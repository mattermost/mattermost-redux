...
export function buildDisplayableChannelList(usersState, teamsState, allChannels, myPreferences) {
    ...
    channels.sort((a, b) => {
        const locale = usersState.profiles[usersState.currentUserId].locale;
    ...
    filter(
        andX(
            isDirectChannel,
            isDirectChannelVisible.bind(null, usersState.currentUserId, myPreferences)
        )
    );
    ...
    return {
        ...
        directChannels: directChannels.filter(
            isConnectedToTeamMember.bind(null, teamsState.membersInTeam[teamsState.currentTeamId])
        ),
        directNonTeamChannels: directChannels.filter(
            isNotConnectedToTeamMember.bind(null, teamsState.membersInTeam[teamsState.currentTeamId])
        )
    };
}
