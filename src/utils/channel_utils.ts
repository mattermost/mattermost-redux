...
export function getGroupDisplayNameFromUserIds(userIds: Array<string>, profiles: IDMappedObjects<UserProfile>, currentUserId: string, teammateNameDisplay: string): string {
    ...
    function sortUsernames(a: string, b: string): number {
        const locale = getUserLocale(currentUserId, profiles);
...
function completeDirectGroupInfo(usersState: UsersState, teammateNameDisplay: string, channel: Channel): Channel {
...
function isDirectChannelForUser(userId: string, otherUserId: string, channel: Channel): boolean {
    return channel.type === General.DM_CHANNEL && getUserIdFromChannelName(userId, channel.name) === otherUserId;
}

function channelHasMentions(members: RelationOneToOne<Channel, ChannelMembership>, channel: Channel): boolean {
...
function buildChannelsWithMentions(channels: Channel[], members: RelationOneToOne<Channel, ChannelMembership>, locale: string): Channel[] {
    return channels.filter(channelHasMentions.bind(null, members)).
        sort(sortChannelsByDisplayName.bind(null, locale));
}

function buildUnreadChannels(channels: Channel[], members: RelationOneToOne<Channel, ChannelMembership>, locale: string): Channel[] {
    return channels.filter(channelHasUnreadMessages.bind(null, members)).
        sort(sortChannelsByDisplayName.bind(null, locale));
}

function getUserLocale(userId: string, profiles: IDMappedObjects<UserProfile>): string {
