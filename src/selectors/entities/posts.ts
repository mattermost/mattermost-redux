...
export function makeGetPostIdsForThread(): (b: GlobalState, a: $ID<Post>) => Array<$ID<Post>> {
    return createIdsSelector(getAllPosts, (state: GlobalState, rootId) => state.entities.posts.postsInThread[rootId] || [], (state: GlobalState, rootId) => state.entities.posts.posts[rootId], (posts, postsForThread, rootPost) => {
        const thread: Post[] = [];
...
export function makeGetPostsChunkAroundPost(): (c: GlobalState, b: $ID<Post>, a: $ID<Channel>) => any {
    return createIdsSelector((state: GlobalState, postId, channelId) => state.entities.posts.postsInChannel[channelId], (state: GlobalState, postId) => postId, (postsForChannel, postId) => {
        if (!postsForChannel) {
            return null;
        }
...
export function makeGetPostIdsAroundPost(): (d: GlobalState, c: $ID<Post>, b: $ID<Channel>, a: {
  ...
  postsAfterCount: number;
    ...
    return createIdsSelector((state: GlobalState, postId, channelId) => getPostsChunkAroundPost(state, postId, channelId), (state: GlobalState, postId) => postId, (state: GlobalState, postId, channelId, options) => options && options.postsBeforeCount, (state: GlobalState, postId, channelId, options) => options && options.postsAfterCount, (postsChunk, postId, postsBeforeCount = Posts.POST_CHUNK_SIZE / 2, postsAfterCount = Posts.POST_CHUNK_SIZE / 2) => {
...
function formatPostInChannel(post: Post, previousPost: Post | undefined | null, index: number, allPosts: IDMappedObjects<Post>, postsInThread: RelationOneToMany<Post, Post>, postIds: Array<$ID<Post>>, currentUser: UserProfile, focusedPostId: $ID<Post>): PostWithFormatData {
    ...
    if (previousPost && previousPost.user_id === post.user_id && post.create_at - previousPost.create_at <= Posts.POST_COLLAPSE_TIMEOUT && !postFromWebhook && !prevPostFromWebhook && !isSystemMessage(post) && !isSystemMessage(previousPost)) {
    ...
    if (currentUser) {
        ...
        isCommentMention = isPostCommentMention({
            post,
            currentUser,
...
export function makeGetPostsInChannel(): (c: GlobalState, b: $ID<Channel>, a: number) => Array<PostWithFormatData> | undefined | null {
    return createSelector(getAllPosts, getPostsInThread, (state: GlobalState, channelId: $ID<Channel>) => getPostIdsInChannel(state, channelId), getCurrentUser, getMyPreferences, (state: GlobalState, channelId: $ID<Channel>, numPosts: number) => numPosts || Posts.POST_CHUNK_SIZE, (allPosts, postsInThread, allPostIds, currentUser, myPreferences, numPosts) => {
        if (!allPostIds) {
            return null;
        }
...
export function makeGetPostsAroundPost(): (c: GlobalState, b: $ID<Post>, a: $ID<Channel>) => Array<PostWithFormatData> | undefined | null {
    ...
    return createSelector((state: GlobalState, focusedPostId, channelId) => getPostIdsAroundPost(state, focusedPostId, channelId, options), getAllPosts, getPostsInThread, (state: GlobalState, focusedPostId) => focusedPostId, getCurrentUser, getMyPreferences, (postIds, allPosts, postsInThread, focusedPostId, currentUser, myPreferences) => {
...
export function makeGetCommentCountForPost(): (b: GlobalState, a: {
  post: Post;
    ...
    return createSelector(getAllPosts, (state, {
        post,
...
export const getSearchResults: (a: GlobalState) => Array<Post> = createSelector(getAllPosts, (state: GlobalState) => state.entities.search.results, (posts, postIds) => {
    if (!postIds) {
        return [];
    }
...
export function makeGetMessageInHistoryItem(type: string): (a: GlobalState) => string {
    return createSelector((state: GlobalState) => state.entities.posts.messagesHistory, (messagesHistory) => {
...
export function makeGetPostsForIds(): (b: GlobalState, a: Array<$ID<Post>>) => Array<Post> {
    return createIdsSelector(getAllPosts, (state: GlobalState, postIds) => postIds, (allPosts, postIds) => {
        if (!postIds) {
            return [];
        }
...
export const getLastPostPerChannel: (a: GlobalState) => RelationOneToOne<Channel, Post> = createSelector(getAllPosts, (state: GlobalState) => state.entities.posts.postsInChannel, (allPosts, postsInChannel) => {
    const ret = {};
...
export const getLatestReplyablePostId: (a: GlobalState) => $ID<Post> = createSelector(getPostsInCurrentChannel, (posts) => {
    if (!posts) {
        return '';
    }
...
export const isPostIdSending = (state: GlobalState, postId: $ID<Post>): boolean => state.entities.posts.pendingPostIds.some((sendingPostId) => sendingPostId === postId);
export const makeIsPostCommentMention = (): (b: GlobalState, a: $ID<Post>) => boolean => {
    return createSelector(getAllPosts, getPostsInThread, getCurrentUser, getPost, (allPosts, postsInThread, currentUser, post) => {
        if (!post) {
            return false;
        }
