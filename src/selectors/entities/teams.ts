...
export const getCurrentTeam = createSelector(getTeams, getCurrentTeamId, (teams, currentTeamId) => {
...
export const isCurrentUserCurrentTeamAdmin = createSelector(getCurrentTeamMembership, (member) => {
    if (member) {
...
export const getCurrentTeamUrl = createSelector(getCurrentUrl, getCurrentTeam, (state) => getConfig(state).SiteURL, (currentURL, currentTeam, siteURL) => {
...
export const getCurrentRelativeTeamUrl = createSelector(getCurrentTeam, (currentTeam) => {
    if (!currentTeam) {
        return '/';
    }
...
export const getCurrentTeamStats = createSelector(getCurrentTeamId, getTeamStats, (currentTeamId, teamStats) => {
    return teamStats[currentTeamId];
});
export const getMyTeams = createSelector(getTeams, getTeamMemberships, (teams, members) => {
    return Object.values(teams).filter((t: Team) => members[t.id] && t.delete_at === 0);
});
export const getMyTeamMember = createSelector(getTeamMemberships, (state, teamId) => teamId, (teamMemberships, teamId) => {
    return teamMemberships[teamId] || {};
});
export const getMembersInCurrentTeam = createSelector(getCurrentTeamId, getMembersInTeams, (currentTeamId, teamMembers) => {
...
}), isCompatibleWithJoinViewTeamPermissions, (teams, myMembers, canListPublicTeams, canListPrivateTeams, compatibleWithJoinViewTeamPermissions) => {
...
export const getListableTeams = createSelector(getTeams, getListableTeamIds, (teams, listableTeamIds) => {
    return listableTeamIds.map((id) => teams[id]);
});
export const getSortedListableTeams = createSelector(getTeams, getListableTeamIds, (state, locale) => locale, (teams, listableTeamIds, locale) => {
    const listableTeams = {};
...
export const getJoinableTeamIds = createIdsSelector(getTeams, getTeamMemberships, (state) => haveISystemPermission(state, {
    permission: Permissions.JOIN_PUBLIC_TEAMS,
}), (state) => haveISystemPermission(state, {
    permission: Permissions.JOIN_PRIVATE_TEAMS,
}), isCompatibleWithJoinViewTeamPermissions, (teams, myMembers, canJoinPublicTeams, canJoinPrivateTeams, compatibleWithJoinViewTeamPermissions) => {
    return Object.keys(teams).filter((id) => {
        const team = teams[id];
        const member = myMembers[id];
...
export const getJoinableTeams = createSelector(getTeams, getJoinableTeamIds, (teams, joinableTeamIds) => {
    return joinableTeamIds.map((id) => teams[id]);
});
export const getSortedJoinableTeams = createSelector(getTeams, getJoinableTeamIds, (state, locale) => locale, (teams, joinableTeamIds, locale) => {
    const joinableTeams = {};
...
export const getMySortedTeamIds = createIdsSelector(getMyTeams, (state, locale) => locale, (teams, locale) => {
    return teams.sort(sortTeamsWithLocale(locale)).map((t: Team) => t.id);
});
export const getMyTeamsCount = createSelector(getMyTeams, (teams) => {
    return teams.length;
...
export const getChannelDrawerBadgeCount = createSelector(getCurrentTeamId, getTeamMemberships, (currentTeamId, teamMembers) => {
    let mentionCount = 0;
    let messageCount = 0;
...
export function makeGetBadgeCountForTeamId() {
    return createSelector(getTeamMemberships, (state, id) => id, (members, teamId) => {
