...
export const getVisibleGroupIds = createSelector(getGroupShowPreferences, (groups) => {
    return groups.filter((dm) => dm.value === 'true' && dm.name).map((dm) => dm.name);
});
export const getTeammateNameDisplaySetting = createSelector(getConfig, getMyPreferences, (config, preferences) => {
...
export const getTheme = createShallowSelector(getThemePreference, getDefaultTheme, (themePreference, defaultTheme) => {
    let theme;
...
export function makeGetStyleFromTheme() {
    return createSelector(getTheme, (state, getStyleFromTheme) => getStyleFromTheme, (theme, getStyleFromTheme) => {
...
export const getSidebarPreferences = createSelector((state: GlobalState) => {
    const config = getConfig(state);
    return config.ExperimentalGroupUnreadChannels !== General.DISABLED && getBool(state, Preferences.CATEGORY_SIDEBAR_SETTINGS, 'show_unread_section', config.ExperimentalGroupUnreadChannels === General.DEFAULT_ON);
}, (state) => {
    ...
    // Support unread settings for old implementation
        sidebarPrefs = {...defaultSidebarPrefs,
