...
export function getTeams(page = 0, perPage: number = General.TEAMS_CHUNK_SIZE, includeTotalCount = false): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            dispatch({
                type: TeamTypes.GET_TEAMS_FAILURE,
                data,
            }, getState);
...
export function joinTeam(inviteId: string, teamId: string): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: TeamTypes.JOIN_TEAM_REQUEST,
            data: null,
        }, getState);
        const state = getState();
        ...
        try {
            ...
            forceLogoutIfNecessary(error, dispatch, getState);
            dispatch(batchActions([{
                type: TeamTypes.JOIN_TEAM_FAILURE,
                error,
            }, logError(error)]));
            return {
                error,
        ...
        dispatch({
            type: TeamTypes.JOIN_TEAM_SUCCESS,
            data: null,
        }, getState);
        return {
            data: true,
