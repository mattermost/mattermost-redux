...
export function init(platform: PlatformType, siteUrl: string | undefined | null, token: string | undefined | null, optionalWebSocket: any, additionalOptions: any = {}) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const authToken = token || Client4.getToken(); // Set the dispatch and getState globally
...
export function doReconnect(now: number) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (currentTeamId) {
            ...
            dispatch(getMyTeamUnreads()); // We need to wait for these actions so that we have an
...
function handleConnecting() {
    doDispatch({
        type: GeneralTypes.WEBSOCKET_REQUEST,
        data: null,
    });
}
...
function handleTeamAddedEvent(msg) {
    return async (dispatch) => {
        await Promise.all([dispatch(getTeam(msg.data.team_id)), dispatch(getMyTeamUnreads())]);
        return {
            data: true,
        };
    };
