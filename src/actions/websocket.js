...
function handlePreferencesChangedEvent(msg, dispatch, getState) {
    ...
    dispatch({type: PreferencesTypes.RECEIVED_PREFERENCES, data: preferences}, getState);
...
function getAddedDmUsersIfNecessary(preferences, dispatch, getState) {
    ...
    for (const preference of preferences) {
        if (preference.category === Constants.CATEGORY_DIRECT_CHANNEL_SHOW && preference.value === 'true') {
    ...
    for (const userId of userIds) {
        ...
        if (statuses[userId] !== Constants.ONLINE) {
...
function handlePreferencesDeletedEvent(msg, dispatch, getState) {
    const preferences = JSON.parse(msg.data.preferences);
    dispatch({type: PreferencesTypes.DELETED_PREFERENCES, data: preferences}, getState);
}
...
function loadPostsHelper(teamId, channelId, dispatch, getState) {
    ...
    if (Object.keys(posts).length === 0 || postsLength < Constants.POST_CHUNK_SIZE || latestPostTime === 0) {
