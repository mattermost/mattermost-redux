...
export function checkMfa(loginId: string): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: UserTypes.CHECK_MFA_REQUEST,
            data: null,
        }, getState);
        ...
        try {
            ...
            dispatch({
                type: UserTypes.CHECK_MFA_SUCCESS,
                data: null,
            }, getState);
            return {
            ...
            dispatch(batchActions([{
                type: UserTypes.CHECK_MFA_FAILURE,
                error,
...
export function loadMe(): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const promises = [dispatch(getMe()), dispatch(getMyPreferences()), dispatch(getMyTeams()), dispatch(getMyTeamMembers()), dispatch(getMyTeamUnreads())]; // Sometimes the server version is set in one or the other
...
export function revokeAllSessionsForUser(userId: string): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const data = {
            isCurrentUser: userId === getCurrentUserId(getState()),
        };
        dispatch(batchActions([{
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        for (let i = 0; i < values.length; i++) {
            ...
            if (member) {
                ...
                } else if ((member.mention_count > 0 || member.msg_count < channel.total_msg_count) && isGroupChannel(channel) && !isGroupChannelVisible(config, myPreferences, channel)) {
...
export function autocompleteUsers(term: string, teamId = '', channelId = '', options: {
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: UserTypes.AUTOCOMPLETE_USERS_REQUEST,
            data: null,
        }, getState);
        const {
            currentUserId,
        ...
        try {
            ...
            forceLogoutIfNecessary(error, dispatch, getState);
            dispatch(batchActions([{
                type: UserTypes.AUTOCOMPLETE_USERS_FAILURE,
                error,
            }, logError(error)]), getState);
            return {
                error,
...
export function updateMe(user: UserProfile): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: UserTypes.UPDATE_ME_REQUEST,
            data: null,
        }, getState);
        let data;
        ...
        try {
            ...
            dispatch(batchActions([{
                type: UserTypes.UPDATE_ME_FAILURE,
                error,
            }, logError(error)]), getState);
            return {
                error,
...
export function clearUserAccessTokens(): ActionFunc {
    return async (dispatch) => {
        dispatch({
            type: UserTypes.CLEAR_MY_USER_ACCESS_TOKENS,
            data: null,
        });
        return {
            data: true,
