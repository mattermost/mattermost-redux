...
export function getPing(): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        let data;
        let pingError = new FormattedError('mobile.server_ping_failed', 'Cannot connect to the server. Please check your server URL and internet connection.');
...
export function getDataRetentionPolicy(): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: GeneralTypes.DATA_RETENTION_POLICY_REQUEST,
            data: {},
        }, getState);
        let data;
...
export function setAppState(state: GeneralState['appState']): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: GeneralTypes.RECEIVED_APP_STATE,
            data: state,
        }, getState);
        return {
            data: true,
...
export function getRedirectLocation(url: string): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        dispatch({
            type: GeneralTypes.REDIRECT_LOCATION_REQUEST,
            data: {},
        }, getState);
